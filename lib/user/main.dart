                ...
                ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      ...
                      // Access the registered users list from AuthAppState
                      AuthAppState? appState = context.findAncestorStateOfType<AuthAppState>();
                      if (appState != null) {
                        ...
                        if (!isSignedIn) {
                          appState.loginUser(email); // Call loginUser to update user state in AuthApp
                        ...
                        // Handle error if AuthAppState is not found
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                ...
                ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      ...
                      // Access the registerUser function from AuthAppState
                      AuthAppState? appState = context.findAncestorStateOfType<AuthAppState>();
                      if (appState != null) {
                        appState.registerUser(newUser); // Register the new user in AuthApp State
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
